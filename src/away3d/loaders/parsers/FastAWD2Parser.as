package away3d.loaders.parsers {
	...
	import away3d.core.base.IndicesRange;
	import away3d.arcane;
	import away3d.containers.ObjectContainer3D;
	import away3d.core.base.BCompositeGeometry;
	import away3d.core.base.BCompositeSubGeomSource;
	import away3d.core.base.BinarySubGeometry;
	 ...
	 */
	public class FastAWD2Parser extends ParserBase
	{
		 ...
		 */
		public function FastAWD2Parser()
		{
...
//				read_uint = _optimized_for_accuracy? _body.readUnsignedInt : _body.readUnsignedShort;
			...
			if( _cursor > 0 ) _body.position = _cursor;
...
//					break;
		...
		private function parseMeshInstance(blockLength : uint) : Mesh
		{
			...
			materials_parsed = 0;
//			trace( "away3d.loaders.parsers.FastAWD2Parser - parseMeshInstance -- ",name , num_materials );
		...
		private function parseMeshData(blockLength : uint) : Geometry
		{
			...
			while (subs_parsed < num_subs) {
				...
				var sub_geom : BinarySubGeometry;
				var compositeGroups : Dictionary;
				
				sub_geom = new BinarySubGeometry();
				...
				while (_body.position < sm_end) {
					...
					if (str_type == 1) {
						sub_geom.updateVertexData(_body, _body.position, str_len/12 );
					}
					else if (str_type == 8 ) {
						isGComposite = 
						isComposite = useComposite;
						var irange : IndicesRange;
						var gid : uint;
						var glen : uint;
						...
						if( useComposite ) {
							...
							while (_body.position < str_end) {
								...
								compositeGroups[gid] = 
								irange = new IndicesRange();
								
								irange.bytesOffset = _body.position;
								irange.numIndices = glen;
								_body.position += (glen<<1);
							}
							
						} else {
//							gindices = new ByteArray();
//							while (_body.position < str_end) {
//								gid = _body.readUnsignedShort();
//								glen = _body.readUnsignedShort();
//								gindices.writeBytes( _body, _body.position, glen<<1 );
//								_body.position += glen<<1;
//							}
//							sub_geom.updateIndexData(gindices);
						...
						sub_geom.updateIndexData(_body, _body.position, str_len>>1 );
					}
					else if (str_type == 3) {
						sub_geom.updateUVData(_body, _body.position);
					}
					else if (str_type == 4) {
						sub_geom.autoDeriveVertexNormals = false;
						sub_geom.updateVertexNormalData(_body, _body.position);
					}
					else if (str_type == 9) {
						sub_geom.updateVertexColorData(_body, _body.position);
					}
					else if (str_type == 6) {
				...
				if( isComposite ) {
					var composite : BCompositeSubGeomSource = new BCompositeSubGeomSource( _body );
					composite.updateVertexData(_body, sub_geom.verticesOffset, sub_geom.numVertices );
					composite.updateUVData(_body, sub_geom.uvsOffset );
					composite.updateVertexNormalData(_body, sub_geom.vertexNormalsOffset );
			...
			if( isGComposite )
				geom = new BCompositeGeometry();
			else
				geom = new Geometry();
			
			for (var i : int = 0; i < subgeoms.length; i++) {
				geom.addSubGeometry( subgeoms[i] );
				if( subgeoms[i] is BCompositeSubGeomSource )
