package away3d.materials.utils
{
	...
	internal class RegisterPool
	{
		 ...
		 */
		public function RegisterPool(type : uint, regCount : int, persistent : Boolean = true)
		{
			_type = type;
			_regCount = regCount;
			_persistent = persistent;
			initRegisters(type, regCount);
		}
		 ...
		 */
		private function initRegisters(type : uint, regCount : int) : void
		{
			
			var hash : String = RegisterPool._initPool( type, regCount );
		...
		private static function _initPool(type : uint, regCount : int) : String
		{
			var hash : String = type.toString(16)+regCount;
			...
			for (var i : int = 0; i < regCount; ++i) {
				vectorRegisters[i] = new ShaderRegisterElement(type, i);

				for (var j : int = 0; j < 4; ++j) {
					registerComponents[j][i] = new ShaderRegisterElement(type, i, j);
				}
			}
			return hash;
		}
